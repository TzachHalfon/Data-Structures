...
void AppendTester()
{
	...
	int i = 0;
	int status = 1;
	slist_ptr_t first_list = SListCreate();
	slist_ptr_t second_list = SListCreate();
	slist_iter iter_first = SListBegin(first_list);
	slist_iter iter_second = SListBegin(second_list);
	...
	printf("AppendTester finished successfully\n");
}
